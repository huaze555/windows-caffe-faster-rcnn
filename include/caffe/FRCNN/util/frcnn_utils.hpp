...
public:
  ...
  string to_string() const {
    char buff[100];
    _snprintf(buff, sizeof(buff), "%.1f %.1f %.1f %.1f", Point[0], Point[1], Point[2], Point[3]);
    return string(buff);
  }
...
public:
  ...
  inline string to_string() const {
    char buff[100];
    _snprintf(buff, sizeof(buff), "cls:%3d,let:%3d -- (%.3f): %.2f %.2f %.2f %.2f", this->id, this->tracklet,
...
public:
  ...
  inline string message() {
    ...
    char buff[100];
    _snprintf(buff, sizeof(buff), "height : %d, width : %d " , this->height, this->width);
    return string(buff);
  }
